...
export default function parseLooty(lootedInfo: string[], allItems: Item[]) {
    ...
    toReturn.obtained = lootedInfo.filter(looted => !filteredLootyItems.includes(looted) && !looted.startsWith("DungW")).map(looted => allItems.find((item) => item.internalName == looted) ?? Item.emptyItem(looted, "Unknown Name"));
