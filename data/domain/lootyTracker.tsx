...
export default function parseLooty(lootedInfo: string[], allItems: Item[]) {
    ...
    toReturn.obtainable = allItems.filter((item) => !filteredLootyItems.includes(item.internalName) && !item.internalName.startsWith("DungW"));
    toReturn.obtained = lootedInfo.filter(looted => !filteredLootyItems.includes(looted) && !looted.startsWith("DungW")).map(looted => allItems.find((item) => item.internalName == looted) ?? new Item({ internalName: looted, displayName: "Unknown Name", Type: "Misc"}));
