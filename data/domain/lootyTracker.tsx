...
export default function parseLooty(lootedInfo: string[], allItems: Item[]) {
    ...
    toReturn.obtained = lootedInfo.filter(looted => !filteredLootyItems.includes(looted)).map(looted => allItems.find((item) => item.internalName == looted) ?? new Item({ internalName: looted, displayName: "Unknown Name", Type: "Misc"}));
