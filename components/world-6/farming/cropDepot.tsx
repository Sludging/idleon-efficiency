...
const SeedSection = ({ seed, farming }: { seed: Seed, farming: Farming }) => {
    return (       
        <ShadowBox width={"auto"} style={{ opacity: seed.data.lvlReq > farming.farmingLevel ? 0.5 : 1 }} margin={{ bottom: 'small' }} background="dark-1" gap="xsmall" pad="small" align="left">
            <Box align="center" direction="row">
                <Text size="medium">{seed.data.name} SEED {seed.data.lvlReq > farming.farmingLevel ? `(Lv required : ${seed.data.lvlReq})` : `(Cycle time : ${toTime(seed.getFullCycleGrowthTime() / farming.growthRate)})`}</Text>
            </Box>
